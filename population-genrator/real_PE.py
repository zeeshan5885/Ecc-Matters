...
for input_file in input_files:
    ...
    Mc_true = (d1 * d2) ** (3.0 / 5.0) / (d1 + d2) ** (1.0 / 5.0)
    ...
    row = 9 / np.power(np.random.uniform(), 1.0 / 3.0)  # SNR
    ...
    alpha = np.min(
        [0.07 / snr_fac, ln_mc_error_pseudo_fisher]
    ...
    if isinstance(eta, float):
        ...
        etaV_sqrt[
            np.logical_not(indx_ok)
        ] = 0  # Set negative cases to 0, so no sqrt problems
