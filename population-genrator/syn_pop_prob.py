...
def joint_rvs(
    N, alpha, m_min, m_max, M_max, sigma_ecc, rand_state=None
    ...
    def rvs(N):  # need to generate rvs with three colums and third should be ecc
        ...
        ecc = st.truncnorm.rvs(0, 1, loc=0, scale=sigma_ecc, size=N)

        return np.column_stack((m_1, m_2, ecc))
...
def joint_pdf(
    ...
    tmp_TS_i1 = np.empty(TS, dtype=np.bool)
    tmp_TS_i2 = np.empty(TS, dtype=np.bool)
...
def pdf_const(alpha, m_min, m_max, M_max, out=None, where=True):
    ...
    tmp_i = np.zeros(S, dtype=np.bool)
