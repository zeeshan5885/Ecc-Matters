...
#
# notes
#    - target low latency operation: fast models, preferably for low chirp mass objects, aggressive grid structures
#    - may do even single-stage codes/very fast extrinsic.  Aim for SPEED, cut corners, minmal iteration, try to get skymap
#    - --assume-precessing required to get a precessing analysis
#    - Iterative: rf and GMM are default operations modes 
#    - flat strategies only
#    - non-iterative: add option to use a single core, no condor, just get something out fast
#
# HISTORY
#    - Based on util_RIFT_pseudo_pipe
#
# EXAMPLES
...
parser.add_argument("--internal-flat-strategy",action='store_true',help="Use the same CIP options for every iteration, with convergence tests on.  Passes --test-convergence.  DEFAULT SHOULD BE  TRUE IN LOW LATENCY ")
...
if opts.l_max > 2:
    print( " ==> NOT Reducing minimum template frequency because of HM (lowlatency mode) <== ")
