...
def test_labels(client, create_obj, obj_type):
    ...
    # Add a label
    label1 = 'DQV'
    response = client.write_label(obj_id, label1)
    ...
    # Add another label
    label2 = 'INJ'
    response = client.write_label(obj_id, label2)
    assert response.status_code == 201
    data = response.json()
    # NOTE: events API returns empty response - should be fixed
    if obj_type == 'superevent':
    ...
    data = response.json()
    obj_labels = [label['name'] for label in data['labels']]
    ...
    # Remove a label
    response = client.remove_label(obj_id, label1)
