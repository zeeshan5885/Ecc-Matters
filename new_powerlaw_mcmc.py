...
def intensity(indiv_params, pop_params, aux_info, M_max=None, **kwargs):
    (
        ...
        indiv_params.T
    )  # adding ecc as a random variable into the model. it would be given in the input data files.
    (
        log_rate,
        alpha,
        m_min,
        m_max,
        sigma_ecc,
    ) = pop_params  # adding sigma_ecc into the population model as a parameters which will be our output.
...
def expval_mc(
    ...
    **kwargs
    ...
    def p(
        N,
    ):  # random number genrator from the pdf defined in the prob.py file and modifed above
        val = prob.joint_rvs(
            N,
            alpha,
            m_min,
...
def log_prior_pop(
    ...
    **kwargs
    ...
    if (
        log_rate_scale == "uniform"
    ):  # I do need the explanation of this if not syntax.?????????
...
def _main(raw_args=None):
    ...
    if (
        raw_args is None
    ):  # I need to know the synatx of main,,, like in the main function, we have said that raw_args=None which is fixed now and then we are adding arg????
        raw_args = sys.argv[1:]
    ...
    for event_fname in cli_args.events:
        ...
        m1_m2_ecc = np.column_stack(  # how it's reading the file and how we are telling him to look for specific data file.
